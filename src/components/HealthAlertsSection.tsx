import { Heart, Activity, Moon, TrendingDown, Shield } from "lucide-react";
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {alerts.map((alert, index) => (
            <AnimatedItem key={index} delay={index * 0.1}>
              <div className={`glass-card rounded-2xl p-6 hover:shadow-elevated transition-all duration-300 border-l-4 ${alert.borderColor} h-full`}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={`${alert.bgColor} rounded-xl p-3 border ${alert.borderColor}`}>
                      <p className="text-xs text-muted-foreground italic leading-relaxed">{alert.example}</p>
                    </div>
                  </div>
                </div>
              </div>
